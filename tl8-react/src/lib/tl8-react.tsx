...
import { TL8TargetAPI, TargetApplicationConfig, WebAppOverwrittenTranslations } from "./copied-from-tl8";

const tl8Target = TL8TargetAPI();

export const useTL8Context = ({ translations, currentLang: defaultCurrentLang }: {
  ...
  const [state, setState] = useState<{ currentLang: string, overwrittenTranslations: Record<string, any> }>(
  ...
  const setOverwrittenTranslations = useCallback((overwrittenTranslations: Record<string, any>) => {
  ...
  }, [config, setOverwrittenTranslations, defaultCurrentLang, translations, setCurrentLang]);
